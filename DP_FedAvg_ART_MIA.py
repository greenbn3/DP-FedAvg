...
class Client:
    def __init__(self, model_fn, dataset, batch_size, learning_rate, device, epsilon=None, delta=1e-5):
        ...
        # If epsilon provided, add DP via Opacus
...
def run_membership_inference_attack_art(global_model, train_dataset, test_dataset, device):
    ...
    for x_batch, _ in train_loader:
        input_shape = x_batch.shape[1:]  # e.g. (1,28,28) for MNIST, (3,32,32) for CIFAR
        break
    ...
    art_classifier = PyTorchClassifier(
        ...
        loss=criterion,
        optimizer=optimizer,
        input_shape=input_shape,  # e.g. (3,32,32) if CIFAR
        nb_classes=10,
    ...
    member_x, member_y = loader_to_numpy(train_loader)    # "in" data
    non_member_x, non_member_y = loader_to_numpy(test_loader)

    # Fit the MIA attack model
    mia_attack.fit(
        x=member_x,
        y=member_y,
...
def log_training_progress(dataset_name, epsilon_str, accuracies, log_dir="./log"):
...
def log_mia_results(dataset_name, epsilon_str, final_acc, in_acc, out_acc, overall_acc, log_dir="./log"):
    os.makedirs(log_dir, exist_ok=True)
...
def main():
    ...
    rounds = 10
    epochs = 2
    epsilons = [None, 0.01, 0.1, 0.25, 0.5, 1.0, 5.0]  # adjust as you like
    delta = 1e-5
